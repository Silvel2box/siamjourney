...
import { getAllProvinces, getAllPlaces, getAllHotels, getAllGuides } from "@/lib/content";

export const revalidate = 3600;

export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const base = site.url;
  const [provinces, places, hotels, guides] = await Promise.all([
    getAllProvinces(),
    getAllPlaces(),
    getAllHotels(),
    getAllGuides(),
  ...
  return [
    ...
    ...(guides.length > 0 ? [{ url: `${base}/guide`, priority: 0.8 }] : []),
