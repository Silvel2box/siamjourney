...
- [x] **เลิกใช้ `import:provinces` บน prod — sync ทีละคอลัมน์แทน ✅ (2026-08-06) — ยังไม่ deploy**
  ...
  - 📌 **ค้าง: ยังไม่รู้ว่า prod มี drift อะไรบ้าง** — ต้อง pull แล้วรัน `report:drift` บน Plesk ก่อน sync · ถ้ารายงานว่าช่องที่แอดมินแก้ (เช่น `body`/`image`/`highlights`) ต่างจาก markdown ต้องตัดสินใจว่า **ดึงค่าจาก prod กลับมาใส่ markdown** (ให้ markdown เป็นความจริงอีกครั้ง) หรือ **ปล่อยให้ DB เป็นเจ้าของช่องนั้นถาวรแล้วไม่ sync มันอีก**
- [x] **Klook รอบ 3 — เก็บ CSV ที่เหลือให้ครบ ✅ (2026-08-06) — ยังไม่ deploy**
