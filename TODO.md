...
- [x] **Shopee: ปุ่ม 77 อันเลิกชี้หน้าแรกเปล่า ✅ (2026-08-06) — ยังไม่ deploy**
  - **CSV `shopee_affiliate.csv` (100 รายการ) ใช้สินค้าไม่ได้เลย** — เป็นลิสต์ "ออฟเฟอร์คอมสูง" ของ Shopee (อาหารเสริม/กลูต้า/แพมเพิส/หูฟัง/แก้ว CIVAGO) · **แมตช์ keyword 77 จังหวัด = 0/77** · คำว่า "ผ้า" ที่เจอ 13 อันเป็นผ้าอ้อม/เสื้อผ้า · เบญจรงค์/จักสาน/สายไหม/เครื่องเงิน/ทอมือ/OTOP = **0 ทุกคำ** → เอามาแปะหน้าของฝากจังหวัด = หลอกคนอ่าน แย่กว่าลิงก์ตายเดิม
  - ⭐ **แต่ CSV ตอบเรื่องที่ค้างมานาน: `ลิงก์ข้อเสนอ` = `https://s.shopee.co.th/XXXX` ครบ 100/100** = shortlink ที่**พก tracking มาในตัวแล้ว** ไม่ต้องเดา `af_siteid` อีก (ซึ่งเดาไว้ในโค้ดตั้งแต่แรกและไม่เคยตรวจ)
  - 🐛 **บั๊กที่เจอเพราะไปดู href จริงบนไลฟ์:** `buildAffiliateUrl` เติม `subIdParam` + UTM **โดยไม่เช็ค `net.id`** (ต่างจากบล็อก `idParam` ที่เช็ค) → ปุ่ม Shopee ทั้ง 77 ยิงออกพร้อม `?af_sub_siteid=<slug>` มาตลอดทั้งที่ยังไม่เคยมี ID · แก้ให้ผูกกับ `net.id` + เพิ่มฟิลด์ `preTracked` แล้วใส่ entry `s.shopee.` **ไว้ก่อน** entry `shopee.` (hostname แมตช์ทั้งคู่ `find()` เอาตัวแรก) → shortlink จะออกไปดิบๆ ไม่โดนต่อท้าย
  - **user เลือกแนว "ครบ 77 ด้วยคำค้นกว้าง"** หลังเห็นทางเลือก (อีกทางคือทำเฉพาะจังหวัดที่มีของจริงแล้วถอดที่เหลือแบบ Klook) · ตัดชื่อจังหวัดออก 14 คำ · **คงไว้ 9 คำที่ชื่อจังหวัดเป็นส่วนหนึ่งของชื่อสินค้าจริง** (เค้กเมืองตรัง · พลอยจันทบุรี · ผ้าไหมสุรินทร์ · โอ่งมังกรราชบุรี · มะขามหวานเพชรบูรณ์ …) เพราะตัดแล้วคำเสียความหมาย · `มุกภูเก็ต`→`ไข่มุก` · **ผลข้างเคียงที่ยอมรับแล้ว: `ผ้าฝ้ายทอมือ` กับ `ผ้าบาติก` ซ้ำกันจังหวัดละคู่**
  - `scripts/apply-shopee-links.mjs` + `scripts/shopee-links.json` — **แยกข้อมูลเป็น json ไม่ inline แบบ Klook** เพราะ 77 แถวและ generate จาก worklist (`E:\Doc-IT\claude-workspace\shopee_worklist.csv`) การพิมพ์มือจะผิดกฎ "ค่าจากไฟล์ข้อมูลห้ามพิมพ์เอง" · `apply-klook-links.mjs:135` มี `if (text.includes("shopee")) continue` อยู่แล้ว **สองสคริปต์จึงไม่ลบบล็อกของกันเอง**
  - 🔴 **ลิงก์ชุดนี้ยังไม่ได้ค่าคอมสักบาท** — เป็น URL ค้นหาเปล่าๆ = แก้ UX ไม่ใช่แก้รายได้ · **สลับเป็น shortlink ทีหลัง = แก้ json แล้วรันสคริปต์ซ้ำ ไม่ต้องแตะอย่างอื่นเลย** · ยังค้าง: ต้องรู้ว่า Affiliate Center สร้างลิงก์จาก URL **หน้าค้นหา** ได้ไหม ถ้าไม่ได้ต้องเลือกสินค้าจริงทีละจังหวัด
  - 🐛 **กับดักที่เกือบพลาด: หน้าเว็บอ่านจาก DB ไม่ใช่ markdown** — แก้ .md + build แล้ว href ที่เรนเดอร์ยังเป็นของเก่า เพราะ `allPlaces()` อ่าน Prisma · **ต้อง `sync:affiliate` ก่อน `build` เสมอ** (กฎเดียวกับ seed/import ที่เคยพลาดมาแล้ว)
  - verify: 77 url+label ตรงกับ json **100% (0 mismatch)** · **การ์ด Klook 14 ใบยังอยู่ครบ** · `tsc` สะอาด · build 730 หน้าเท่าเดิม · href ที่เรนเดอร์จริงถอดคำค้นออกมาถูก (โรตีสายไหม · ตุ๊กตาชาววังบางเสด็จ · ผ้าไหมสุรินทร์ · โอ่งมังกรราชบุรี) · **`af_sub_siteid` เหลือ 0 ครั้งทั้งเว็บ** · klook wrapper ไม่เปลี่ยน
  - **DEPLOY (ไม่มี migration):** `pull → report:drift → sync:affiliate → build → restart`
- [x] **เลิกใช้ `import:provinces` บน prod — sync ทีละคอลัมน์แทน ✅ LIVE + verify ไลฟ์ (2026-08-06)**
  - **ที่มา: user ถามว่า "แก้ข้อมูลผ่านแอดมินไว้ ถ้า deploy แล้วหายไหม"** → อ่านโค้ดแล้วคำตอบคือ **หาย** — `import-content.mjs` เป็น upsert ที่ `update: row` = เขียนทับ **13 คอลัมน์ทั้ง 77 แถว** (`image` `imageCredit` `highlights` `bestTime` `gettingThere` `localFood` `body` `tours` …) · user ยืนยันว่าที่แก้ผ่านแอดมินคือ **หน้าจังหวัด** = ตรงกับที่โดนทับพอดี
  - `scripts/report-drift.mjs` (`report:drift`) — **อ่านอย่างเดียว ไม่เขียนอะไรเลย** เทียบ DB กับ markdown เฉพาะคอลัมน์ที่ตัวเขียนจะแตะจริง แล้วรายงานว่าแถวไหน/ช่องไหนจะหาย · `scripts/sync-tours.mjs` (`sync:tours`) — เขียนคอลัมน์ `tours` คอลัมน์เดียว แมตช์ด้วย slug (คู่ขนานกับ `sync:affiliate` เป๊ะ)
  - 🐛 **`npm run sync:tours -- --dry` บน PowerShell กลืน `--dry` ทิ้งแล้วเขียนจริง** — สัญญาณเดียวที่จับได้คือ output ไม่มี prefix `[dry]` · **กระทบ `sync:affiliate --dry` ที่มีอยู่เดิมด้วย** → เพิ่ม npm script แยก **`sync:tours:dry` / `sync:affiliate:dry`** (Plesk "Run script" ส่ง argument ไม่ได้อยู่แล้ว จึงต้องมี script แยกอยู่ดี)
  - verify: เทสตรรกะเทียบค่า 16 เคส (**CRLF↔LF และลำดับ key ใน object ไม่นับเป็น drift** ไม่งั้นรายงานเต็มไปด้วย false alarm · แต่**ลำดับใน array นับ** เพราะลำดับทัวร์มีความหมาย) · **เทสกับ DB จริง: แกล้งแก้ `bestTime` + ลบ `tours` เลียนแบบแอดมิน → report จับได้ 2 ช่อง → `sync:tours` คืน `tours` ให้ แต่ `bestTime` ที่ "แอดมินแก้" ยังอยู่ครบ** ← คุณสมบัติที่ต้องการทั้งหมด · คืนค่า DB + ลบไฟล์เทสเรียบร้อย
  - แก้คอมเมนต์หัว `import-content.mjs` ที่เดิม **แนะนำให้รัน `--provinces` บน prod** (คำแนะนำที่จะทำงาน user หาย) → LOCAL ONLY ทั้งสองโหมด · `DEPLOY.md` เพิ่มหัวข้อ "อัปเดตเนื้อหาจาก markdown ขึ้น prod" + gotcha `--dry`
  - ⭐ **ผล `report:drift` บน prod จริง — คุ้มค่าที่ทำ:** drift ที่เป็นงานแอดมินมี **1 จุดเดียวคือ `bangkok.featured` = true** (markdown เป็น `false` มาตั้งแต่ initial import) → **ถ้ารัน `import:provinces` กรุงเทพฯ หลุดจากหน้าแรกทันที** · ที่เหลือ 5 จังหวัดเป็น `tours` ของใหม่ล้วน · **10 จังหวัดที่มีทัวร์อยู่แล้วไม่โผล่เลย = การ re-quote label เป็น no-op บน prod จริง** · places: 3 การ์ดใหม่ + **`2 in DB only` (`gravity-amnat-charoen`, `wat-phra-that-pha-sorn-kaew`) ที่ user เพิ่มเองในแอดมิน ถูกข้ามตามดีไซน์** · แก้ปมด้วยการบันทึก `featured: true` กลับเข้า markdown (`7d8c087`) — ไม่กระทบไลฟ์เพราะไม่มีสคริปต์ไหนเขียนคอลัมน์นี้
  - verify ไลฟ์หลัง deploy: `sync:tours` = 5 set/72 correct/**with tours=15** · `sync:affiliate` = 3 set/305 correct/**places=310** (308 markdown + 2 ของแอดมิน) · **หน้าแรกยังมีจังหวัดแนะนำ 9 รายการ รวม `/central/bangkok`** = งานแอดมินรอดจากการ deploy จริง ✅
- [x] **Klook รอบ 3 — เก็บ CSV ที่เหลือให้ครบ ✅ LIVE + verify ไลฟ์ (2026-08-06, `d82e145`)**
  - อยุธยา 6 ทัวร์ + การ์ดวัดมหาธาตุ · **สุราษฎร์ธานี 2→6 เต็มเพดาน** (อ่างทอง 2 + ATV X-Quad/ซิปไลน์ Tree Bridge/สปาศาลาราย/wellness แอบโซลูท แซงชัวรี) · **สมุทรสงคราม + เพชรบุรี เปิดบล็อกใหม่จังหวัดละ 1** · แม่ฮ่องสอน 1 · การ์ดหน้าสถานที่เพิ่มสมุย + อัมพวา → **บล็อกทัวร์ 15/77 จังหวัด · การ์ด Klook 14 ใบ**
  - ⭐ **ออดิตแล้วว่า Klook ตันจริง:** CSV 15 ไฟล์ = **สินค้าไม่ซ้ำ 240 ใช้ไป 68** · ที่เหลือไม่ได้ใช้เพราะ **เพดาน 6/จังหวัด** ไม่ใช่ของตกหล่น (เชียงใหม่ 42 ใช้ 7 · ภูเก็ต 52 ใช้ 8 · กระบี่ 41 ใช้ 6) · **ทางขยายที่เหลือมีทางเดียวคือ export จังหวัดใหม่** แต่จังหวัดรองได้แค่ 1-2 ใบ (ตราด 2 · หาดใหญ่ 2 · แม่ฮ่องสอน 1 · เพชรบุรี 1 · สมุทรสงคราม 1)
  - 🔴 **`klook_af_link_อ่างทอง.csv` ไม่ใช่ของจังหวัดอ่างทอง** — เป็น**หมู่เกาะ**อ่างทองซึ่งอยู่ในสุราษฎร์ธานี (City ในไฟล์เขียนสมุย/สุราษฎร์ฯ ทั้ง 3 แถว) → ลง `surat-thani` ทั้งหมด · **หน้าจังหวัดอ่างทองยังไม่มีทัวร์ และ CSV นี้ช่วยไม่ได้เลย**
  - 🐛 **กับดักชื่อสินค้า vs ที่ทัวร์ไปจริง (ซ้ำรอย `49271`):** `88330` ชื่อบอกแค่ "ทัวร์ตลาดน้ำและวัดปากน้ำ" + City=กรุงเทพฯ → เกือบตัดทิ้งว่าผิดจังหวัด · **Klook ตอบ 403 ให้ bot ทุกทาง** (WebFetch/curl/API ทั้ง `/th/` และ `/en-US/`) จึงยืนยันด้วย 2 ทางแทน: itinerary หน้าสินค้าเดียวกันบน Traveloka (MaeKlong Railway Market + ดำเนินสะดวก + วัดปากน้ำ ไม่มีอัมพวา) และ **เปิดรูปสินค้าที่ Klook ส่งมาใน feed ดูเอง = ภาพรถไฟแทรกกลางตลาดร่มหุบชัดๆ** → จุดแรกอยู่สมุทรสงครามจริง · **label เขียนเองให้ตรงของจริง ไม่ใช้ชื่อดิบจาก CSV** ไม่งั้นคนอ่านไม่รู้ว่ามาโผล่หน้านี้ทำไม
  - 🔴 **label ต้อง quote:** ชื่อทัวร์แม่ฮ่องสอนมี `": "` กลางประโยค → frontmatter พังเงียบๆ (importer จับได้) · ใส่ `q()` ในสคริปต์ทั้งสองตัว ทำให้ไป re-quote ของเดิม 10 จังหวัด/11 สถานที่ด้วย · **พิสูจน์แล้วว่าเป็น no-op เชิงความหมาย** (parse ค่าเก่าเทียบใหม่เท่ากันทุกไฟล์) → `sync:tours` จึงไม่เขียนซ้ำจังหวัดที่ไม่ได้เปลี่ยนจริง
  - verify: ลิงก์ Klook ในคอนเทนต์ 81 อัน เทียบกลับ CSV = **72 ตรงเป๊ะทั้ง url+image · 9 ตรวจไม่ได้** เพราะ CSV รอบแรก (กรุงเทพฯ/กาญจนบุรี/เพชรบูรณ์) ไม่อยู่บนดิสก์แล้ว — **ยืนยันด้วย git ว่าทั้ง 9 เหมือนใน HEAD ทุกไฟล์ = ของที่ commit+live ไปแล้ว ไม่ได้แตะรอบนี้** · `tsc` สะอาด · build **730 หน้าเท่าเดิม** · อ่าน HTML ที่ prerender จริง นับ id ไม่ซ้ำจาก `k_site`: surat-thani 6 สินค้า/6 รูป · สมุทรสงคราม 1/1 · เพชรบุรี 1/1 · อยุธยา 6/6
  - **DEPLOY (ไม่มี migration):** `pull → report:drift (ดูก่อน!) → sync:tours → sync:affiliate → build → restart` — 🔴 **ห้าม `import:provinces`**
  - ✅ **verify ไลฟ์:** 5 หน้าจังหวัดตอบ 200 + จำนวนทัวร์ตรงเป๊ะ (อยุธยา 6 · สุราษฎร์ฯ 6 · สมุทรสงคราม 1 · เพชรบุรี 1 · แม่ฮ่องสอน 1) นับจาก id ไม่ซ้ำใน `k_site` · การ์ด 3 หน้าสถานที่ตรงสินค้า (`2508` / `11542` / `319`) · **รูปพาร์ทเนอร์ผ่าน optimizer จริง** = `200 image/webp` + `max-age=31536000`
- [x] **เพิ่มจังหวัด Klook รอบ 2: เชียงใหม่ + หัวหิน/หาดใหญ่/เชียงราย/พัทยา ✅ LIVE + user เทสผ่าน (2026-08-05, `0f53cf1`+`3780d5a`)**
  - **การ์ดหน้าสถานที่ 10 · บล็อกทัวร์หน้าจังหวัด 8** — กรุงเทพฯ/เชียงใหม่/เชียงราย/ชลบุรี/กระบี่/ภูเก็ต/ประจวบคีรีขันธ์ อย่างละ 6 ทัวร์ · **สงขลา 2** (CSV หาดใหญ่มีสินค้าแค่ 2 จริงๆ — บล็อกแสดงเท่าที่มี ไม่เติมของให้ครบ)
  - ✅ **ปิดปม `49271`** (URL บอก white-temple แต่ชื่อสินค้าบอกทัวร์แม่น้ำ+แถมซิม) — CSV เชียงรายมีทัวร์ที่ชื่อ+URL ตรงกันและระบุวัดร่องขุ่นทั้งคู่ → `66954` · verify ไลฟ์: `49271` = 0 ทั้งเว็บ
  - ✅ **หาดพัทยาเลิกยืมทัวร์ที่ออกจากกรุงเทพฯ** → `1168` ทัวร์เกาะล้านจากพัทยา · เขาสามร้อยยอด → `74146` ชื่อตรงเป๊ะ · ดอยสุเทพ → `18014`
  - ⚠️ **CSV แต่ละไฟล์ไม่เหมือนกัน:** เชียงใหม่/หัวหิน/พัทยา = **USD และไม่มี `aff_label1` เลย** · ที่เหลือ THB + `sj01` · **ถ้าก๊อปลิงก์จาก CSV มาแปะตรงๆ คนไทยจะเห็นราคาดอลลาร์ และคลิกไม่ถูกนับให้เรา** — ของเราเอาแค่ `k_site` มาประกอบใหม่เสมอจึงไม่กระทบ (เหตุผลที่ตัดสินใจถูกตั้งแต่แรก)
  - **หาดสมิหลาไม่มีการ์ด — ปิดเรื่องแล้ว (2026-08-15): Klook ไม่มีสินค้าของสมิหลา** (ผู้ใช้ยืนยันหลังค้นในแผงพาร์ทเนอร์) · CSV หาดใหญ่มีแต่ Bouncetopia/พิพิธภัณฑ์มายากลซึ่งเป็นของในเมือง ไม่เกี่ยวกับหาด → **ตั้งใจปล่อยให้หน้าหาดสมิหลาไม่มีปุ่ม** ตามกฎเดิม: ปุ่มต้องพาไปที่สิ่งที่ป้ายบอก ไม่ใช่ของใกล้เคียงที่พอมี
  - verify: ทุก url/image generate จาก CSV แล้วเทียบกลับ **54 ลิงก์ ไม่ตรง 0** · ไลฟ์ทุกหน้า จำนวนทัวร์ = จำนวนรูป · รูปใหม่ `200 image/webp`
  - ⭐ **commit กับ deploy เป็นคนละเรื่อง** — commit ทีละจังหวัดได้ (diff เล็ก ย้อนง่าย) แล้ว pull ทีเดียวตอนท้าย · **ทำ 3-4 จังหวัดแล้ว deploy รวดเดียวปลอดภัยกว่า** (เจอหน้าต่าง ISR ค้างน้อยครั้ง)
  - **DEPLOY (ไม่มี migration แล้ว):** `pull → sync:affiliate → import:provinces → build → restart`
  - 📌 **เหลือ (อัปเดต 2026-08-15):** สุราษฎร์ธานี + อยุธยา ✅ · **สงขลา ปิดแล้ว** — สองการ์ดหาดใหญ่ (Bouncetopia + พิพิธภัณฑ์มายากล `41392`) ขึ้นไลฟ์ตั้งแต่ 5 ส.ค. และ Klook ไม่มีของสมิหลา = **CSV หาดใหญ่ถูกใช้ครบทุกแถวแล้ว** · **ฝั่ง Klook จึงถือว่าจบ** เหลือแต่ Shopee (กรอกไป 10/234 แถว) ที่ยังเป็นงานรายได้ที่ค้างอยู่จริง
- [x] **บล็อกทัวร์ในหน้าจังหวัด ✅ LIVE + verify ไลฟ์ (2026-08-05, `40a20af`)**
  - **ที่มา: user ถามว่า "กระบี่ติด affiliate ครบทุกลิงก์ไหม"** → ตรวจแล้วพบว่า CSV 3 ไฟล์มี **132 สินค้า แต่เว็บใช้ได้แค่ 8** เพราะลิงก์ Klook มีที่อยู่ที่เดียว = ปุ่มบนหน้าสถานที่เด่น จังหวัดละ 1 · **หน้าจังหวัดซึ่งอยู่บนกรวยกว่า ไม่มีลิงก์ affiliate เลยสักอัน**
  - `Province` + `tours Json?` = `[{label,url,image?}]` (migration `add_province_tours`) · เรนเดอร์เป็นกริดด้วย `AffiliateButton` ตัวเดิม · **6 การ์ด/จังหวัด: กระบี่ · ภูเก็ต · กรุงเทพฯ** · จังหวัดที่ไม่มีทัวร์ = ไม่มีบล็อก
  - **จำกัด 6 ใน zod** — เกินกว่านี้เลิกอ่านเป็น "แนะนำไม่กี่อัน" กลายเป็นแผงโฆษณา · มีประโยคเปิดเผยว่าได้ค่าตอบแทนและผู้อ่านจ่ายเท่าเดิม · `ToursField` แอดมินแก้เองได้ (ไม่มีปุ่มอัปโหลดไฟล์ ต่างจาก GalleryField เพราะรูปต้องเป็นของพาร์ทเนอร์)
  - ⭐ **`AffiliateButton` เลิกล็อก `max-w-2xl` ในตัวเอง ย้ายไป call site** — component ที่ hardcode ความกว้างตัวเองลงกริดไม่ได้
  - 🔴 **บทเรียนสำคัญสุด — เกือบปล่อยข้อมูลที่แต่งเอง:** ดราฟต์แรก**พิมพ์ hash รูปของภูเก็ต/กรุงเทพฯ ขึ้นมาเอง** (ดึงจริงมาแค่ 8 อัน) → ถ้าปล่อยไปรูป 10 ใบ 404 หมด · แก้เป็น generate map จาก CSV + verify เทียบทุก url/image กับไฟล์ต้นทาง (18/18) · **กฎ: ค่าที่มาจากไฟล์ข้อมูลภายนอก ห้ามพิมพ์เอง ให้ generate แล้ว verify กลับเสมอ**
  - 🐛 **`prisma generate` ติด EPERM เพราะ dev server ล็อก engine** — สื่อสารพลาด user ไปเปิด `npm run dev` เพิ่ม เสียเวลา 2 รอบ ต้องบอกให้ชัดว่า **"ปิด"** · ปิดเองด้วย `Get-CimInstance Win32_Process | Where CommandLine -like "*siamjourney*"` → `Stop-Process` (กรองด้วยชื่อโปรเจกต์ ไม่แตะโปรเจกต์อื่นที่รันคู่กัน) แล้ว**บอก user ให้เปิดใหม่**
  - verify ไลฟ์: ทัวร์ 6 + รูป 6 ไม่ซ้ำ ครบ 3 หน้า · อำนาจเจริญไม่มีบล็อก · รูป klook ผ่าน optimizer = `200 image/webp` cache 1 ปี
  - 🔴 **DEPLOY (มี migration):** `pull → NPM install → migrate:deploy → sync:affiliate → import:provinces → build → restart`
- [x] **Klook affiliate ✅ LIVE (2026-08-05, `508d47e`→`6df1858`)**
  - **8 จังหวัดมีปุ่มจริง** (จาก CSV 2 ไฟล์ รวม 91 สินค้า): กรุงเทพฯ→วัดพระแก้ว · เชียงราย→วัดร่องขุ่น · เพชรบูรณ์→เขาค้อ · พังงา→เขาตาปู (4 อันนี้ตรงชื่อสถานที่เป๊ะ) · กาญจนบุรี · ชลบุรี · ภูเก็ต · กระบี่ · **อีก 69 จังหวัดถอดปุ่มออก** (ปุ่มพาไปหน้าแรกเปล่าแย่กว่าไม่มีปุ่ม)
  - ⭐ **การ์ดสินค้า (`6df1858`) — user ทักเองว่า CSV มีคอลัมน์ `Product Image` แต่ไม่ได้เอามาใช้** · `Affiliate` เพิ่ม `image?` (column เป็น `Json?` = **ไม่ต้อง migration**) → มีรูป=การ์ด (รูป+ชื่อ+ป้าย "พาร์ทเนอร์ · โดเมน"+CTA) / ไม่มีรูป=ปุ่มกลมเดิม · แอดมินกรอกได้ทั้ง PlaceForm+HotelForm
  - **hotlink `res.klook.com` ไม่ self-host (สวนกฎรูปของโปรเจกต์โดยตั้งใจ):** รูปเป็นของผู้ประกอบการ + Klook ปล่อยใน feed ให้ affiliate ใช้ + **เขาเปลี่ยนรูปสินค้าเมื่อไหร่ การ์ดตามเอง** · ยังผ่าน next/image = ได้ WebP
  - 🔴 **ไม่แสดง `Sell Price` ทั้งที่ CSV มี** — ราคาขยับและไม่มีอะไรรีเฟรช = ปัญหาเดียวกับ `check_in` ที่ฝังในลิงก์โรงแรม · **ป้าย "พาร์ทเนอร์" จำเป็นเมื่อมีรูป** เพราะการ์ดดูเหมือนคอนเทนต์ที่เราเขียนเอง (ชื่อดึงจาก hostname ไม่ฮาร์ดโค้ด)
  - **สคริปต์ 2 ตัว:** `apply-klook-links.mjs` (object `PRODUCTS` = source of truth · แก้ text ตรงๆ ไม่ใช้ gray-matter · **รันซ้ำได้สองทาง ใส่บล็อกกลับได้** → รอบหน้าแค่เพิ่มใน PRODUCTS แล้วรัน) · `sync-affiliate.mjs` = `npm run sync:affiliate` (push **เฉพาะ field `affiliate`** match by slug)
  - 🐛 **บั๊กในสคริปต์ตัวเอง:** `count({ where: { affiliate: { not: Prisma.DbNull } } })` **คืนทุกแถว** บน `Json?` ของ MySQL — เกือบรายงาน "308 มี affiliate" ทั้งที่จริง 82 → นับใน JS แทน
  - 📌 **ค้างให้ user เช็ค:** สินค้าวัดร่องขุ่น `49271` ข้อมูล CSV ขัดกันเอง — URL=`visit-white-temple...` แต่ชื่อสินค้า="(ฟรี ซิมการ์ด) ทัวร์ครึ่งวันชมสถานที่ห้ามพลาดรอบแม่น้ำ"
  - 🔴 **DEPLOY (ไม่มี migration แต่มีขั้นเพิ่ม):** `pull → Run script: sync:affiliate → build → restart` — **`sync:affiliate` ต้องรันก่อน `build`** (กฎเดิม: เขียน DB ต้องจบก่อน build ไม่งั้น ISR ค้าง snapshot) · **ห้าม `import:content`** (prod 309 places จะทับงานแอดมิน)
- [x] **Klook affiliate: กลไกลิงก์ ✅ (2026-08-04, `145558a`+`f234802`)**
  - **สิ่งที่เจอและใหญ่กว่าตัว ID: ลิงก์ affiliate 154 อัน (Klook 77 + Shopee 77) มี 153 อันชี้ "หน้าแรกเปล่า"** — ปุ่มเขียน "จองทัวร์บึงกาฬ" แล้วโยนคนไป `klook.com/th/` · **กรอก ID เฉยๆ = เปิดท่อที่ปลายไม่ต่อกับอะไร**
  - **🔑 Klook ใช้ redirect wrapper ไม่ใช่แปะพารามิเตอร์ท้าย URL ปลายทาง** (สมมติฐานเดิมในโค้ดผิดสนิท): `https://affiliate.klook.com/redirect?aid=129762&_currency=THB&k_site=<urlencode ปลายทาง>&aff_label1=<sub id>`
  - แก้: `NetworkCfg` เพิ่มฟิลด์ `redirectBase?` — มีค่า=ห่อ / ไม่มีค่า=แปะแบบเดิม (Shopee/Lazada/Agoda/Booking ไม่กระทบ)
  - 🔴 **`aff_label1` ตั้งเองไม่ได้ ต้องเป็นแท็กที่สร้างในแดชบอร์ดก่อน** (`f234802`) — ส่งค่าที่ Klook ไม่รู้จัก **ไม่ error แต่ทิ้งคนไว้หน้าแรกเงียบๆ** (ลองใส่ slug `bangkok-wat-phra-kaew` แล้วเด้งหน้าแรกจริง) → ใช้ฟิลด์ `fixedSubId: "sj01"` แท็กเดียวทั้งเว็บ · **ยอมเสีย attribution รายสถานที่** เพราะ GA4 วัด outbound click ต่อหน้าให้อยู่แล้ว (ที่เสียคือรู้ว่าคลิกไหนปิดการขาย ซึ่งทราฟฟิกยังไม่พออ่าน) · 📌 ยังไม่ได้ถาม Klook ว่ามี `aff_label2/3` ที่กรอกอิสระได้ไหม
  - ✅ verify: **เทียบสตริงดิบกับลิงก์ที่แดชบอร์ด export มา → เหมือนทุกไบต์ 39/39** + slug ไม่หลุดเข้าลิงก์ Klook · `tsc` สะอาด · **user กดยืนยันแล้วเข้าหน้าบัตรพระบรมมหาราชวังถูกต้อง**
  - ⭐ **บทเรียน verify (พลาดมาแล้ว 1 รอบ):** รอบแรกเทียบ**ทีละพารามิเตอร์**แล้วสรุปว่าใช้ได้ — ค่าที่ปลายทางไม่ยอมรับมัน "ตรงช่อง" ได้โดยผิดความหมาย → **ลิงก์ที่ต้องให้บริการนอกยอมรับ ต้องเทียบสตริงดิบกับตัวอย่างที่เขาออกให้เอง แล้วให้ user กดจริงก่อนประกาศว่าใช้ได้**
  - 🔴 **klook.com บล็อกบอต — `curl`/WebFetch ได้ 403 ทุก URL รวมหน้าแรกเขาเอง** → ยืนยัน URL ก่อน deploy เองไม่ได้ ต้องให้ user กดในเบราว์เซอร์เสมอ
  - 🔴 **คอลัมน์ City ใน CSV เชื่อไม่ได้ = จุดออกเดินทาง ไม่ใช่ปลายทาง** (ทัวร์เขาค้อ→เพชรบูรณ์ และทัวร์วัดร่องขุ่น→เชียงราย ถูกแปะ "กรุงเทพมหานคร" ทั้งคู่) → ต้องแกะจาก URL สินค้า · ⚠️ ลิงก์ hotel ฝัง `check_in`/`check_out` ตายตัว (จะกลายเป็นวันที่ในอดีต ต้องถอด) · มีสินค้า commission 0.000% ปนมา ต้องคัดทิ้ง
  - **เหลือ (รอ CSV เมืองท่องเที่ยวเพิ่ม):** ใส่ลิงก์สินค้าจริง → ถอดปุ่ม Klook จากจังหวัดที่ไม่มีของ → สคริปต์อัปเดต **เฉพาะ field `affiliate`** ใน DB · 🔴 **ห้ามใช้ `import:content` เต็มบน prod** (prod 309 places vs local 308 — จะทับงานแอดมิน) ใช้ guard แบบ `--provinces`
  - CSV รอบแรก 39 สินค้า แกะได้ 5 จังหวัด: กรุงเทพฯ→วัดพระแก้ว · เชียงราย→วัดร่องขุ่น · เพชรบูรณ์→เขาค้อ (3 อันนี้ตรงสถานที่เป๊ะ) · กาญจนบุรี→สะพานข้ามแม่น้ำแคว · ชลบุรี→หาดพัทยา
