...
- [ ] 🚧 **Klook affiliate — โค้ดเสร็จ รอข้อมูล (2026-08-04, `145558a` · ยังไม่ deploy)**
  ...
  - แก้: `NetworkCfg` เพิ่มฟิลด์ `redirectBase?` — มีค่า=ห่อ / ไม่มีค่า=แปะแบบเดิม (Shopee/Lazada/Agoda/Booking ไม่กระทบ) · `aff_label1` = **slug ของสถานที่** ไม่ใช่ `sj01` ตายตัวที่แดชบอร์ด export มา (ไม่งั้นคลิกทั้งเว็บกองถังเดียว ดูไม่ออกว่าหน้าไหนทำเงิน)
  - verify: เอา 39 ลิงก์จริงจาก CSV แกะ `k_site` ให้โค้ดสร้างใหม่ → **ตรง 39/39** ทุกพารามิเตอร์ ต่างแค่ `aff_label1` (ตั้งใจ) · `tsc` สะอาด · **user กดลิงก์วัดพระแก้วที่ generate เองแล้วใช้งานได้จริง**
