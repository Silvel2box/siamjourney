...
- [ ] **2A — Foundation: ย้าย content → DB** ⭐ ทำก่อน (ทุกอย่างต่อยอดจากนี้ + เสี่ยงสุด = de-risk ก่อน)
  - Prisma model `Place` + `Province` (imageCredit/affiliate เก็บเป็น Json คง shape เดิม)
  - importer `scripts/import-content.mjs`: markdown 385 → DB (upsert idempotent, validate ด้วย zod เดิม, เก็บ .md เป็น backup)
  - `lib/content.ts` อ่านจาก DB (getter เป็น async + React cache, คง signature)
  - หน้า static → **ISR** (dynamicParams=true + revalidate) รองรับ add/edit โดยไม่ rebuild
  - verify: เว็บ render เหมือนเดิมเป๊ะจาก DB · prod: `migrate:deploy` + รัน importer 1 ครั้ง
- [ ] **2B — Admin CRUD สถานที่/จังหวัด** — list + form เพิ่ม/แก้/ลบ (หมวด/จังหวัด/พิกัด/affiliate/sponsored/เนื้อหา) + revalidatePath ตอน save + role admin/editor
- [ ] **2C — อัปโหลดรูปผ่าน admin** 🔑 ต้องเลือกที่เก็บ: persistent dir บน Plesk (ฟรี ระวัง backup/deploy) vs object storage (Cloudinary/Bunny) — widget upload + resize sharp + media library
