...
export default function HotelForm({
  ...
  const [state, formAction, pending] = useActionState<State, FormData>(saveHotel, null);
  ...
  return (
    <form action={formAction} className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field id="name" label="ชื่อโรงแรม / ที่พัก *">
        ...
        <Field id="slug" label={mode === "edit" ? "slug (แก้ไม่ได้)" : "slug * (a-z 0-9 -)"}>
          <input
            ...
            placeholder="เช่น chiang-mai-riverside-resort"
      ...
      <ImageUploadField name="image" label="รูปโรงแรม *" defaultValue={values.image} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Field id="priceRange" label="ช่วงราคา (ต่อคืน)">
          <input id="priceRange" name="priceRange" defaultValue={values.priceRange} placeholder="เช่น ฿900–2,500" className={input} />
        </Field>
      </div>
      ...
      <fieldset className="border border-gray-200 rounded-2xl p-4">
        <legend className="text-sm font-medium text-gray-600 px-2">ลิงก์จองที่พัก / affiliate (ถ้ามี)</legend>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field id="affiliateLabel" label="ข้อความปุ่ม">
            <input id="affiliateLabel" name="affiliateLabel" defaultValue={values.affiliateLabel} placeholder="เช่น จองที่พัก (Agoda)" className={input} />
          </Field>
      ...
      >
        {pending ? "กำลังบันทึก..." : mode === "create" ? "เพิ่มที่พัก" : "บันทึกการแก้ไข"}
      </button>
    </form>
  );
}
