...
> 🔴 **`import:guides` ไม่ใช่ขั้นตอนประจำของ deploy — รันเฉพาะรอบที่ `content/guides/*.md` เปลี่ยน**
> (คนแก้ md จะเขียนกำกับไว้ใน note ของ deploy รอบนั้น) · deploy รอบอื่น = `pull → build → restart` เฉย ๆ
> เหตุผล: มันเป็น upsert ที่ `update` ทับ `body` ของบทที่มีไฟล์ md **ทุกครั้ง** → ถ้าทีมแก้บทเดิมหรือใส่รูป
> ผ่าน `/admin/guides` แล้วเผลอรัน งานนั้นหายเงียบ ๆ (เคยเกิดแล้ว 2026-08-11) · บทที่ทีมเขียนเองในแอดมิน
> ไม่มีไฟล์ md จึงไม่ถูกแตะอยู่แล้ว
> 🔴 รอบที่ต้องรัน ให้รัน **ก่อน** `build` เหมือนทุกอย่างที่เขียน DB ไม่งั้น `/guide` ค้างแคชเก่า 1 ชม.
